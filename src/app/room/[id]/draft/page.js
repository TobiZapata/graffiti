...
export default function MultiplayerDraftPage({ params }) {
  ...
  const handleReady = async () => {
    ...
    await submitDraft(roomId, user.uid, squad, teamName.trim() || "My Team");
          ...
          {draftCompleted ?
            <div className="flex h-150 flex-col items-center justify-center gap-6">
              ...
              <button
                ...
                className="rounded-xl bg-green-600 px-12 py-6 text-3xl font-bold hover:bg-green-700 transition-colors disabled:opacity-50"
              >
                READY
              </button>
            </div>
