...
export default function SimulationUI() {
  ...
  // In multiplayer, my personal match
  const myMatch =
    currentRoundMatches.find(
      (m) =>
        m.playerUids?.includes(
          userUid,
        ) && !m.completed,
    );
  // Any human match pending (to block auto-advance)
  const anyHumanMatchPending =
    currentRoundMatches.some(
      (m) =>
        m.isPlayerMatch && !m.completed,
    );

  // If matches started and I have a match, force me to the match screen
  useEffect(() => {
    if (
      matchesStarted &&
      myMatch &&
      ...
      router.push(
        `/room/${roomId}/match?id=${myMatch.id}`,
      );
    }
  }, [
    matchesStarted,
  ...
  useEffect(() => {
    // If the host is here and matches were started, but everyone finished their match, advance the round.
    if (
      isHost &&
      ["swiss", "playoffs"].includes(
        stage,
      ) &&
      matchesStarted &&
      !anyHumanMatchPending
    ) {
      advanceRound();
    }
  }, [
  ...
  const getMatchesByPool = () => {
    ...
    matches.forEach((m) => {
      if (typeof m.round !== "number")
        return; // solo matches swiss (round numérico)
          ...
          {isHost &&
            ...
            !matchesStarted && (
              <button
                ...
                className={`rounded-lg px-6 py-3 font-bold text-sm uppercase tracking-wider transition-all hover:scale-105 shadow-lg ${
                  anyHumanMatchPending ?
                    "bg-green-600 hover:bg-green-500 shadow-green-600/30"
                  : "bg-neutral-600 hover:bg-neutral-500 shadow-neutral-600/30 text-white"
                }`}
              >
                ▶{" "}
                {anyHumanMatchPending ?
                  "START ROUND MATCHES"
                : "SIMULATE NPC MATCHES"
                }
              </button>
            )}
          {!isHost &&
            ...
            !matchesStarted && (
              <div className="text-sm font-bold text-neutral-400">
                ...
                : "Waiting for host to simulate NPC matches..."
                }
              </div>
            )}
        </header>
...
function ChevronArrows({ direction }) {
  ...
  return (
    <div
      className={`flex flex-col items-center ${color} opacity-60`}
    >
      ...
      <div className="flex items-center gap-0 min-w-[900px] py-4">
        ...
        <div className="flex flex-col justify-center h-[500px] w-[180px]">
          ...
          : <div className="border-2 border-dashed border-neutral-700 rounded-xl p-4 text-center">
              <div className="text-neutral-600 font-bold text-sm">
                TBD
              </div>
...
function BracketMatchCard({ m }) {
  ...
  return (
    <div
      className={`border rounded-lg overflow-hidden ${m.isPlayerMatch ? "border-amber-500 shadow-md shadow-amber-500/20" : "border-neutral-700"} bg-neutral-900`}
    >
      {/* Team A */}
      <div
        className={`flex items-center gap-2 px-3 py-2 ${
          isWinnerA ? "bg-green-900/20"
...
function TeamSummary({
  ...
  if (finalMatch) {
    ...
    // Función simple para generar un rating determinista basado en los stats y el id del jugador
    const getPseudoRating = (p) => {
      ...
      for (
        ...
        hash += p.name.charCodeAt(i);
      const baseStat =
        ((p.aim || 80) +
          (p.gamesense || 80)) /
        2;
      const boost = (hash % 40) / 100; // 0.00 a 0.39
      return baseStat / 70 + boost; // Ej: (85/70) + 0.2 = 1.21 + 0.2 = 1.41
    };
